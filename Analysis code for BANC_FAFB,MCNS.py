"""
Fast solver with inverted-index connection signatures.
Instead of computing conn_sig for each candidate individually (O(N × cands)),
use adjacency lists to build all signatures in one pass (O(N × avg_degree)).
This is 20-100× faster per step, enabling many more random restarts.
"""
import csv
import random
import sys
